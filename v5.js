...
module.exports = function () {
    ...
    if (!Game.creeps.harvester) {
        code = Game.spawns.Spawn1.spawnCreep([MOVE, CARRY, WORK, WORK], "harvester", {
            memory: {
    ...
    if (!Game.creeps.harvester1) {
        code = Game.spawns.Spawn1.spawnCreep([MOVE, CARRY, WORK, WORK], "harvester1", {
            memory: {
                role: Creep.ROLE_HARVESTER,
                state: Creep.STATE_NOTHING,
                prevState: Creep.STATE_NOTHING
        ...
        if (code !== OK) {
            console.log("Fail spawn creep 'harvester1 -", code);
        }
    }
    ...
    for (const creepName of Object.keys(Game.creeps)) {
        ...
        switch (creep.memory.role) {
            ...
            case Creep.ROLE_REPAIRER:
                ...
                break;
            case Creep.ROLE_HARVESTER:
                creepRole = new Harvester(creep);
                break;
            default:
