module.exports[Creep.ROLE_HARVESTER] = {};
module.exports[Creep.ROLE_HARVESTER][Creep.TODO_HARVEST] = function(creep){
    ...
    if(dropped != null){
        ...
        if(status == ERR_FULL){
            creep.todo = Creep.TODO_TRANSFER;
        }else if(status == ERR_NOT_IN_RANGE){
            creep.moveTo(dropped);
        }else if(status == OK){
            creep.say("👇", true);
...
module.exports[Creep.ROLE_HARVESTER][Creep.TODO_TRANSFER] = function(creep){
    ...
    if(status == ERR_NOT_ENOUGH_ENERGY) creep.todo = Creep.TODO_HARVEST;
    else if(status == ERR_NOT_IN_RANGE) creep.moveTo(target);// OPTIMIZE: reusePath, ignoreCreeps
