/**
 * @param {{targetId: string, id: number, requiredCreeps: number}} options
 * @description upgrade controller
 * */
function MissionA(options){
    this.namePrefix = "uc";
    this.id = options.id;
    this.options = options;
    this.quantityCreeps = 0;
}

MissionA.name = "A";

MissionA.prototype.execute = function () {
    let target = Game.getObjectById(this.options.targetId);
    let targetRoom = target.room;
    /**
     * @type {StructureSpawn}*/
    let spawnerRoom = null;

    if(spawnerRoom == null) {
        for (let i in Game.spawns) {
            if (target.room.name === Game.spawns[i].room.name) {
                spawnerRoom = Game.spawns[i];
            }
        }

        if(spawnerRoom == null){
            Game.notify("I can't find spawner in "+targetRoom.name);
            return;
        }
    }

    if(this.quantityCreeps < this.options.requiredCreeps){
        spawnerRoom.spawnCreep([MOVE, WORK, CARRY], (this.namePrefix+"-"+this.id+"-"+Math.floor(Math.random()*100)), {});
    }

    for(let i in Game.creeps){
        let creep = Game.creeps[i];
        let name = creep.name.split("-");
        if(name[0] === this.namePrefix && name[1] === this.id){
            if(creep.memory.canUpgrade){
                let status = creep.upgradeController(target);
                if(status === ERR_NOT_IN_RANGE){
                    creep.moveTo(target);
                }else if(status === ERR_NOT_ENOUGH_RESOURCES){
                    creep.memory.canUpgrade = false;
                }
            }else{
                let sources = spawnerRoom.room.find(FIND_SOURCES_ACTIVE);
                let status = creep.harvest(sources[0]);
                if(status === ERR_NOT_IN_RANGE){
                    creep.moveTo(sources[0]);
                }
            }

            if(creep.store[RESOURCE_ENERGY] === creep.store.getCapacity()){
                creep.memory.canUpgrade = true;
            }
        }
    }
};

module.exports = {
    create(name, options){
        switch (name) {
            case "A":
                return new MissionA(options);
        }
    }
};